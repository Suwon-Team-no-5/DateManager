...
namespace DateManager
{
    public partial class Form1 : Form
    {
        ...
        private void btnPlay_Click(object sender, EventArgs e) // 재생 버튼 토글 기능
        {
            ...
            // 재생 시작 시, 아무것도 선택되지 않았다면 0번 인덱스부터 시작
        ...
        // ====================================================================
        // 2. 키보드 단축키 핸들러 (Space 충돌 해결 및 P키 매핑)
        // ====================================================================
        private void Form1_KeyDown(object? sender, KeyEventArgs e)
        {
            ...
            // 💡 [변경] 스페이스바는 재생 중 멈추지 않고 다중 선택(블록 지정)을 수행하도록 리스트박스에 양보합니다.
            // 대신 재생/일시정지 토글은 'P' 키로 명확하게 분리하여 충돌을 원천 차단합니다.
            if (e.KeyCode == Keys.P)
            {
                if (!_isViewingTrash)
                {
        ...
        // ====================================================================
        // 3. 타이머 및 리스트 프레임 선택 파트 (다중 선택 보존 처리 핵심)
        // ====================================================================
        private void PlaybackTimer_Tick(object? sender, EventArgs e)
        {
            ...
            // 💡 타이머 틱에 의해 자동 이동 중임을 증명하는 플래그를 세워, 기존 다중 선택이 풀리지 않도록 보호합니다.
        ...
        private void SelectFrame(int index)
        {
            ...
            // 💡 핵심 버그 수정: 사용자가 의도적으로 구간 선택 중이거나 타이머 실행 중일 때는 
            // 기존 다중 선택 영역(파란 블록)을 강제로 Clear하지 않고 누적 선택되도록 분기 처리합니다.
            ...
            try
            {
                // 현재 가리키는 프레임을 선택 상태에 추가/변경
                lstFrameData.SetSelected(safeIndex, true);
